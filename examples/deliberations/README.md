...
This directory holds **checked-in directory-structured deliberation records** kept as examples or historical references. Live `/committee` runs write to `<situation-dir>/deliberations/`, not here. The review skill reads from situation directories at runtime; this directory exists so the repo can preserve representative records and the `example/` template files.
