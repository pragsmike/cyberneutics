...
| **Conceptual coherence** | 2 | First-use definitions mostly present (eigenforms in 04, situation-gap-bridge in 03, fan/funnel in 10). Terminology stable across core concepts. Issues: "narrative engine" and "stochastic imps" used interchangeably without acknowledgment; "coherent narrative" used extensively in 01 without rigorous definition; forward dependencies (eigenforms referenced in 01, defined in 04; rhizome mentioned in 03, explained in 06). |
...
- **Exposition**: "Game within the game" introduced intuitively but lacks formal definition. Committee characters named via winning conditions but operational detail deferred.
- **Accessibility**: Best for Practitioners frustrated with LLMs. Too anecdotal for Theorists.
- **Issues**: Robert's Rules mentioned but not explained. Missing operational detail on how to actually "play." Conclusion ("truer stories") contradicts thesis that stories aren't about truth.
...
**stories-all-the-way-down.md** — "Everything is narrative" argued through legal, mathematical, and risk examples. Issue: the claim is stated more strongly than defended; needs qualification to complex/sociotechnical domains.

**the-stochastic-imps-of-happenstance.md** — Game-theoretic framing plus character reference manual. Issue: character backgrounds are atmospheric but don't explain how to instantiate the personalities in practice.

**tilt-sound-collective-story.md** — Narrative dramatization of methodology in action (five people taking over a post-production studio). Issue: AI's role appears as black box — we don't see actual output or how recommendations map to decisions.
...
Minor but persistent: "narrative engine" vs. "stochastic imps" vs. "statistical ghost" for the same concept (LLM generating plausible text). "Game within the game" vs. "adversarial committee" vs. "structured deliberation." Used interchangeably without explicit acknowledgment that these are synonyms.
...
1. **Add an explicit handoff from essays to artifacts.** At the end of Essay 02 (or 05, where the synthesis is complete), add a paragraph: "You now have the theoretical grounding. To run the methodology, see [artifacts/start-here.md]." Currently the reader must discover this independently.
...
5. **Acknowledge synonym sets.** In Essay 01 or essays/README.md, note: "We use 'narrative engine,' 'stochastic imps,' and 'statistical ghost' to refer to the same phenomenon: an LLM generating plausible text by traversing latent space." Same for game-within-the-game / adversarial committee / structured deliberation.
...
|---|--------|-----------------|--------|--------------|
| 1 | Add essay→artifact handoff paragraph in Essay 02 and/or 05 | Actionability | Small | None |
...
| 5 | Add synonym acknowledgment (narrative engine / stochastic imps / statistical ghost) | Coherence | Small | None |
