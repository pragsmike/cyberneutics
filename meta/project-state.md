...
### Residuality essay-13 readiness audit + remediation (2026-04-25)

Committee deliberation in [`../situations/residuality-essay-13-readiness/deliberations/`](../../situations/residuality-essay-13-readiness/deliberations/) — full 00–06 record with charter, deliberation, evaluation 1 (10/15, below threshold), remediation round 1 using **isolated fresh-context character agents** (procedural innovation to address premature unanimity), and evaluation 2 (13/15, at threshold, PASS). Edit pass executed on residuality material to address committee recommendations:

- **Spine reframed.** "Convergence as narrative proof" softened across the residuality material (README, bilateral note §3 and §4.7, assemblage-rhizome-nomad §4, essays/README.md essay-13 forward-pointer) to "one seam in cyberneutics' [narrative-proof](../wild/diary/2026-04-03-narrative-proof.md) web — narrow because n=2 with overlapping intellectual environments." Phase 1 evidence check confirmed "narrative proof" is a defended cyberneutics concept (defined in [diary 2026-04-03](../wild/diary/2026-04-03-narrative-proof.md), used in [essays/primacy-of-reasoning-artifacts.md](../essays/primacy-of-reasoning-artifacts.md)) — kept as term, narrowed as claim. Tied to the falsifier paragraph in [`assemblage-rhizome-nomad.md`](../wild/residuality-theory/assemblage-rhizome-nomad.md) as the test for the load-bearing "Deleuzian grammar does explanatory work neither tradition could do alone" formulation.
- **"Cluster" replaced with "multi-document set."** README §"Essay-13: planned graduation as multi-document set" commits to one numbered essay + one practitioner-facing artefact (fan-as-stressor-generator) + survey + scaffolding (bilateral note, assemblage-rhizome-nomad) under deletion test once the essay is drafted.
- **Two-axis classification (evidence tier × stance) applied to bilateral note's §5 summary table.** Replaces the two-value Status column. New tier breakdown: 4 contributions at (a) demonstrated, 2 at (b) implemented but unmeasured, 2 at (c) proposed but not implemented, 3 at (d) speculative or theoretical only. Two items (diary discipline; failure catalog) reclassified as research directions about cyberneutics' own practice — prescriptive stance disqualifies them from the contribution-back inventory. Two new contribution candidates added (situation-directory pattern as residue-from-live-process at (a); decision-landscape mapping as NKP generalization at (b)).
- **Honesty pass on empirical claims.** Status lines added in three places: fan-as-stressor-generator's Empirical claim section (untested protocol); bilateral note §2.2 Eigenform Ri (untested); bilateral note §2.8 calibration register (architectural commitment dependent on multi-agent substrate maturation, not yet implemented).
- **2026-04-20 diary restructured** as Reader's note + Current status + Original entry (2026-04-20). Inline retraction markers removed; the section-level "Note" headers point to the Current status section at the top.

**Remaining open work** (from committee resolution): item 9a (stability inventory document); item 9b (one-session stability window before drafting); item 11 (falsifier paragraph for assemblage-rhizome-nomad mapping); item 17 (numbered-essay meta-decision: choose Posture A/B/C/D); item 18 (empirical protocol run — fan-as-stressor-generator Ri_F − Ri_A or eigenform-Ri on a Black Swan deliberation). Plus Stacey's arrival for the cybernetics-critique answer.

**Decision space** (per committee): four postures available — A (draft now from stable findings); B (wait for Stacey + stability window + protocol run); C (partial draft, defer Stacey-dependent sections); D (graduate as non-essay artefact set, leave essay-13's numbered slot open; precedent: committee-games as bridge paper, March 2026). mg's choice pending.

**Action document for future agents**: [`wild/residuality-theory/essay-13-readiness-plan.md`](../wild/residuality-theory/essay-13-readiness-plan.md). Codifies the audit + remediation outcomes, names what is done and what remains (items 9a, 9b, 11, 17, 18 plus Stacey), the four-posture decision space, the three sustained dissents (Maya procedural / Frankie convergence-framing / Joe drafting-prerequisites), and the canonical contribution-back inventory. Linked from `wild/residuality-theory/README.md` as the first entry in the source-material navigation table.
