...
This is also where the framework's human gate earns its theoretical weight. In the pipeline architecture developed in [Essay 09](./09-narrative-immune-systems.md) and [Essay 10](./10-decisions-under-uncertainty.md), the human gate is the point where pipeline output is committed to or revised — the terminus of the recursive evaluation loop. The account here provides the epistemological argument: the human gate is not merely prudent, it is load-bearing. Without it, the pipeline's outputs are not yet meaning — they are structured potential in search of grounding. The human gate is where potential becomes sense.
