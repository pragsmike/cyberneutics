...
```mermaid
flowchart TD
    classDef epistemic fill:#f9f0ff,stroke:#9370db,stroke-width:2px;
    classDef generative fill:#e6f7ff,stroke:#1890ff,stroke-width:2px;
    classDef adversarial fill:#fff2e8,stroke:#ff7a45,stroke-width:2px;
    classDef topological fill:#f6ffed,stroke:#52c41a,stroke-width:2px;
    classDef consequence fill:#fff0f6,stroke:#eb2f96,stroke-width:2px;

    %% LAYER 1: Epistemic Genesis & Alignment
    subgraph Epistemic_Layer [1. The Trust Commons & Alignment]
        Intent[Business/AI Intent] --> ADR[Cryptographic ADR & Event Storming]
        ADR --> Firewall{Alignment Firewall}
        Firewall -- Coerced/Unverified Seams --> Reject[Reject: Signal Hijack Detected]
        Firewall -- Cryptographically Verified --> DDD
    end
    class Epistemic_Layer epistemic

    %% LAYER 2: Architectural Economics
    subgraph Generative_Layer [2. Bounded Context & Generative AI]
        DDD{Select Bounded Context}
        DDD -- Core Domain --> Functional[Enforce Strict Immutability & High Token Density]
        DDD -- Supporting Domain --> Imperative[Allow Standard CRUD & Boilerplate]
        Functional --> Planner[AI / Human Planner Loop]
        Imperative --> Planner
    end
    class Generative_Layer generative

    %% LAYER 3: Adversarial Friction & The Veto
    subgraph Adversarial_Layer[3. Adversarial Friction & Operator Veto]
        Planner --> Tester{Adversarial Tester}
        Tester -- Ontological Fuzzing / Chaos --> Planner
        Tester -- Survives Fuzzing --> Operator{The Operator}
        Operator -- Exceeds Complexity Cap / Blast Radius --> Ledger{Risk Ledger}
        Ledger -- Escrow Funded --> Deploy
        Ledger -- Budget Bankrupt --> Block[Block: Margin Call Triggered]
        Operator -- Within Residue Matrix --> Deploy[Deploy to Canary Cell]
    end
    class Adversarial_Layer adversarial

    %% LAYER 4: Reality & Macro-Resilience
    subgraph Topological_Layer [4. Physical Reality & Macro-Resilience]
        Deploy --> Reality{Hyperliminal Stress Exerts Force}
        Reality -- Normal Operation --> Value[Value Extracted & Token Replenished]
        Reality -- Unexpected Stress / Failure --> Isolation{Topological Isolation}
        Isolation -- Fails to Contain --> Catastrophe[Systemic Collapse - Axioms Failed]
        Isolation -- Safely Contained / Load Shed --> Residue[Graceful Degradation to Core Residue]
    end
    class Topological_Layer topological

    %% LAYER 5: Micro-Fragility & Apathy Prevention
    subgraph Consequence_Layer [5. The Hammurabi Consequence Loop]
        Residue --> Hammurabi{Hammurabi Pipeline Triggered}
        Hammurabi --> Tax[Waste Ledger: Future Tokens Taxed]
        Hammurabi --> Lock[Environment Locked for Proof-of-Resolution]
        Hammurabi --> Mirror[Epistemic Mirror: Blast-Radius Broadcast]
        Tax -. Feedback Loop .-> Planner
        Lock -. Restores Discipline .-> Planner
    end
    class Consequence_Layer consequence
```
