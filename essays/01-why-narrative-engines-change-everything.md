...
This observability has independent value. Even if the committee does not outperform simpler approaches on decision quality — which is a separate empirical question under active investigation — the inspectable deliberation record is a product in its own right: an audit trail showing which arguments were considered, which were challenged, and what survived. Solo evaluation can reach good decisions without producing this trail. The committee always produces it.
