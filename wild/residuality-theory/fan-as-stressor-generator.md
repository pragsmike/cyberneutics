...
Residual analysis, as described in O'Reilly's 2020 *Introduction to Residuality Theory*, the 2022 *Random Simulation and Attractor Networks* paper, and the 2024 *Residues* book, takes a stressor list as input. The literature describes several techniques for producing the list — playfulness, removal of probability/impact filters, deliberate inclusion of impossible and irrelevant events — and the 2024 book recommends three upstream aids that have proven useful in practice: PESTLE analysis, the Business Model Canvas, and Porter's Five Forces (O'Reilly 2024, *Walking with Stressors / Coaching Stressor Analysis*). What the literature does not specify is a *breadth discipline* — a procedure for generating a list whose coverage of the stressor space is itself defensible, rather than left to the integration of these aids in the architect's own head.

This is the part of residual analysis that practitioners report as hardest. The 2022 paper acknowledges the failure mode directly: "Traditional methods in software architecture viewed as random simulations would seem to suffer from the curse of dimensionality — distributions tend to return to areas of high probability." Even with PESTLE/BMC/5-Forces in hand, a stressor list produced by a single architect drifts toward what that architect can imagine — which is itself a probability-weighted distribution shaped by their experience. Residual analysis works in proportion to the breadth of the stressor list, and the breadth is hard to defend without a discipline for generating it that does not depend on the integrating intuition of the practitioner.
...
What residuality theory needs is a discipline for stressor *generation* that operates upstream of bagging and downstream of the architect's choice of upstream aids — one that produces a measurably broader list than any single integration of PESTLE, BMC, and Porter's Five Forces by an individual architect would. This is the gap the fan operation is offered to fill: not a vacuum where O'Reilly leaves nothing, but a missing breadth-discipline atop the aids he already names.
