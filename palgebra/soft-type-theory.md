...
**Product quantale for vector scores.** The scalar quantale V captures
pipeline-level confidence. Individual rubrics produce richer scores: five
criteria, each 0–3. The natural structure is a product quantale
V^5 = ([0,3]^5, componentwise min, (3,3,3,3,3)). The presheaf construction
generalises directly, but the interaction with the enrichment (which
collapses to the scalar V at pipeline boundaries) needs explicit treatment.
...
presheaf is an open comparison.
...
**Sheaf condition.** The presheaf of §2 is not yet a *sheaf*. A sheaf
condition would say: if an artifact's type grades are locally consistent
(compatible on every overlap of refinement chains), then they extend to a
globally consistent type profile. Whether the rubric system satisfies this
gluing condition — and whether enforcing it would improve pipeline
reliability — is an open question with practical consequences for rubric
design.
